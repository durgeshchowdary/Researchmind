...
class IndexingService:
    def __init__(self) -> None:
        ...
        self.vector_index: faiss.IndexFlatIP | None = None
    ...
    def load_persisted_state(self) -> None:
        ...
        try:
            ...
            if self.faiss_path.exists() and self.chunk_map_path.exists():
                ...
                self.chunk_id_map = json.loads(self.chunk_map_path.read_text(encoding="utf-8"))
            ...
            logger.exception("Failed to load persisted indexes. Reverting to empty in-memory state.")
            self.inverted_index = {}
            self.doc_freqs = {}
            self.chunk_lengths = {}
    ...
    def rebuild_indexes(self) -> None:
        ...
        self.avg_chunk_length = sum(chunk_lengths.values()) / len(chunk_lengths) if chunk_lengths else 0.0

        self._persist_bm25()
        self._rebuild_vector_index(chunk_ids, texts)
        self._mark_chunked_documents_as_indexed()
        observability_service.record_latency("indexing", (time.perf_counter() - started) * 1000)
    ...
    def _mark_chunked_documents_as_indexed(self) -> None:
        ...
        with get_db() as conn:
            before = conn.execute("SELECT COUNT(*) AS count FROM documents WHERE status IN ('chunked', 'indexed')").fetchone()
            conn.execute(
                """
                UPDATE documents
                SET status = 'indexed',
